...
class Processor(IO):
        ...
        if self.arg.phase == 'train':
            self.data_loader['train'] = torch.utils.data.DataLoader(
                ...
                num_workers=self.arg.num_worker * torchlight.ngpu(
                    self.arg.device),
                drop_last=True)
        if self.arg.test_feeder is not None:
            Feeder = import_class(self.arg.test_feeder)
        ...
        if self.arg.phase == 'train':
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if ((epoch + 1) % self.arg.save_interval == 0):
                    ...
                    self.io.print_log('Save model: {}'.format(filename))
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--test_feeder', default=None, help='data loader will be used')
        ...
        parser.add_argument('--seed', type=int, default=7, help='fix seed')
